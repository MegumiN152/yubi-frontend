...
const MyChartPage: React.FC = () => {
  ...
  const initializeSSE = () => {
    ...
    const eventSource = new EventSource(request.baseURL + `/api/sse/user/connect?userId=${currentUser.id}`);
      ...
      >
        {selectedItem && (
          <Form form={form} onFinish={handleSubmit}>
            <Form.Item label="图表id" name="id">
              <Input/>
            </Form.Item>
            <Form.Item label="图表名称" name="name">
              <Input/>
            </Form.Item>
            <Form.Item label="分析目标" name="goal">
              <Input/>
            </Form.Item>
        ...
        renderItem={(item) => (
          <List.Item key={item.id}>
            <Card style={{width: '100%'}}>
              ...
              />
              <>
                {item.status === 'wait' && <Result status="warning" title="待生成"
                                                   subTitle={item.execMessage ?? '当前图表生成队列繁忙，请耐心等候'}/>}
                {item.status === 'running' && <Result status="info" title="图表生成中" subTitle={item.execMessage}/>}
                {item.status === 'succeed' && (
                  <>
                ...
                {item.status === 'failed' && <Result status="error" title="图表生成失败" subTitle={item.execMessage}/>}
              </>
            </Card>
          </List.Item>
        )}
